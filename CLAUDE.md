...
A semantic search system that embeds documents using Voyage AI's contextualized embeddings and stores them in ChromaDB for similarity search. The system is designed to process personal knowledge bases, code repositories, and conversations with intelligent chunking strategies.
...
# - Installs dependencies (voyageai, chromadb, langchain-text-splitters, etc.)
...
- Collections: `knowledgebase`, `code`, `conversations`
- Persistent storage in `./chroma_db_v3/`
...
├── chroma_db_v3/           # Vector database storage
...
python -c "import chromadb; client = chromadb.PersistentClient(path='./chroma_db_v3'); print([c.name for c in client.list_collections()])"
```
